...
const BestOfflineGame = () => {
    const {games} = useContext(GameContext);
    const [bestOfflineGame, setBestOfflineGame] = useState([]);

    useEffect(()=>{
        const bestOfflineGame = games.filter((item) => (item.online === false));
        setBestOfflineGame(bestOfflineGame.slice(0,5))
        },[]);

  return (
    <div className='mt-8'>
      <div className="text-center text-3xl py-4">
        <Title text1={'BEST'} text2={'Offline Game'} />
      ...
      <div className='m-8'>
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6'>
            ...
            {
              bestOfflineGame.map((item, index) => (
                // console.log(item),
...
export default BestOfflineGame
